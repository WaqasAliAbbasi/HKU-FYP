...
function formatDuration(duration, spaced) {
  ...
  if (type === "string") return duration;
...
function prepareSeries(data, noMatchColor, ignoreFilterParam) {
  ...
  if (
    !ignoreFilter &&
    seriesFilter &&
    (!filtersOnlySampleSeries || result.supportsControllersDiscrimination)
  ) {
...
function setGraphZoomable(graphSelector, overviewSelector) {
  ...
  overview.bind("plotunselected", function () {
    ...
    zoomPlot(
      graph.data("plot"),
      overviewAxes.xaxis.min,
      overviewAxes.xaxis.max,
      overviewAxes.yaxis.min,
      overviewAxes.yaxis.max
    );
  });
}
