...
resource "null_resource" "yolo-machine2" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "docker pull waqasali/fyp-yolo",
      "docker run --name yolo-service-6 --cpuset-cpus=25	--pid=host -p ${var.yolo6_port}:50051 --gpus device=0 -d waqasali/fyp-yolo",
      "docker run --name yolo-service-7 --cpuset-cpus=26 --pid=host -p ${var.yolo7_port}:50051 --gpus device=1 -d waqasali/fyp-yolo",
      "docker run --name yolo-service-8 --cpuset-cpus=27 --pid=host -p ${var.yolo8_port}:50051 --gpus device=2 -d waqasali/fyp-yolo",
      "docker run --name yolo-service-9 --cpuset-cpus=28 --pid=host -p ${var.yolo9_port}:50051 --gpus device=3 -d waqasali/fyp-yolo",
      "docker run --name yolo-service-10 --cpuset-cpus=29 --pid=host -p ${var.yolo10_port}:50051 --gpus all -d waqasali/fyp-yolo",
    ]
  }
